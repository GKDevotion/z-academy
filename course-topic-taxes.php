<?php 
include_once ('elements/header.php');
?>

    <header class="container-fluid d-flex align-items-center justify-content-center text-center bg-light hero-section">
        <div>
            <h1 class="display-3 fw-800 animate__animated animate__fadeInDown">
                Taxes <span style="color: var(--zed-primary)"></span>
            </h1>
            <p class="lead mb-4 animate__animated animate__fadeInUp animate__delay-1s">
                
            </p>
            <div class="d-none open-account-btn account-type-btn animate__animated animate__zoomIn animate__delay-1s">
                <button class="rounded-pill">ZERO TO HERO</button>
            </div>
        </div>
    </header>

    <div class="container py-5">

        <!-- Introduction to U.S. Corporate Bonds -->
        <div class="col-12">
            <div class="course-card">
                <span class="badge-level">Beginner</span>
                <div class="row g-3">
                    <div class="col-md-4">
                        <img src="https://picsum.photos/400/300?2" style="height: 150px;" class="course-img">
                    </div>
                    <div class="col-md-8">
                        <h5 class="course-title">Taxes</h5>
                        <p class="course-desc">
                            Traders and investors can learn about a host of tax-related concerns – whether a U.S., or non-U.S., resident. Course material available on Traders’ Academy covers taxes that concern dividends, capital gains and losses, net investment income tax, tax treatment of traditional and Roth individual retirement accounts (IRAs), as well as tax-related topics for those who reside outside of the U.S., such as Form W-8BEN, tax treaties, and more.
                        </p>
                        <div class="row d-none">
                            <div class="col-md-6">
                                <ul class="course-list">
                                    <li><i class="fa fa-chevron-right"></i></li>
                                </ul>
                            </div>
                            <div class="col-md-6">
                                <ul class="course-list">
                                    <li><i class="fa fa-chevron-right"></i></li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
       
    </div>
    
    <footer class="bg-secondary py-5 mt-5 border-top border-danger border-4">
        <div class="container text-center text-white py-4">
            <h2 class="fw-800 mb-3">Unlock <span style="color: var(--prinary-teal)">Elite</span> Access</h2>
            <p class="text-white small mx-auto mb-4" style="max-width: 500px;">Get full curriculum access, live trade webinars, and our private institutional sentiment dashboard.</p>
            <button class="btn btn-lg btn-danger rounded-pill px-5 fw-bold text-uppercase fs-7">Upgrade Now</button>
        </div>
    </footer>
<?php
include_once ('elements/footer.php');
?>